...
The app can automatically fills in some of the data we need by analyzing the
...
rest breaks, where the driver left his vehicle but did notd
deactive the app.
...
app runs. This has several disadvantages. It pushes the responsibility for
...
Alternatively, we could turn our R code into a web service and avoid most of these
pitfalls!
...
* By defining an API i can clearly formalize the boundaries between two components
  of system, for example in our case the geoprocessing service and the mobile app.
  This  often corrsepond to borders between teams as in our case
  where the IT unit maintains the mobile app and we the geoprocessing
  service.
* HTTP has been around for a long time, is well standardized and so practically
  all programming languages can use HTTP APIs.
*  Since the web service
  runs on a dedicated machine, I can control the R version, operating system,
  R packages, system libraries, etc to use. 
* I can easily deploy different version of the service, for example development,
...
* The service is reusable from other projects. For example, I have also
  ...
  view all routes submitted with the mobile app.
...
Now lets look at the system from an architecture p[oint of few. We have several
...
* our geoprocessing service communicate with an external routing service
via an HTTP API

* So it's pretty straight forward that we also choose an HTTP API to expose
our R-based service.
...
Here is a code example for how to define the server side code with plumber.
We define a normal R function that we augment wit special comments that
are then turned into an API specification by plumber. This part here
defines the so called endpoint of our api. The id gets passed to our
R function as a variable. Plumber also passes a special "Response" object
named "res" into the function that we can manipulate. This is purely
optional, but in this case we use it to return the proper HTTP error
codes if we encounter a problem. If you want to learn more about HTTP 
...
from R or from the command line, or from practically any other programming
language. In R we can use the httr library to send HTTP requests. httr
...
* Usually an API has several endpoints. This is a partial overview of the
  endpoints our geoprocessing API provides. Note that there are more types
  ...
  API will be a mess and hard to use for other people. Please refer to the
  link here for a detailed description of HTTP methods.
...
* The plumber package makes it easy to 
